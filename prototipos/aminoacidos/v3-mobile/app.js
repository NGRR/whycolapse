...
const mobile = { active:false, panning:false, last:{x:0,y:0}, padding:96 };
const aminoDefs = [
...
function isMobileView(){const r=stage.getBoundingClientRect();return r.width<720 || r.height<620;}
function applyPan(dx,dy){organism.x+=dx;organism.y+=dy;state.amino.forEach(a=>{a.x+=dx;a.y+=dy;});state.events.forEach(ev=>{ev.x+=dx;ev.y+=dy;});state.sprouts?.forEach(sp=>{sp.x+=dx;sp.y+=dy;});state.memories.forEach(m=>{m.x+=dx;m.y+=dy;});}
...
function resize(){const rect=stage.getBoundingClientRect();bg.width=rect.width*devicePixelRatio;bg.height=rect.height*devicePixelRatio;bg.style.width=rect.width+'px';bg.style.height=rect.height+'px';ctx.setTransform(devicePixelRatio,0,0,devicePixelRatio,0,0);linksSvg.setAttribute('viewBox',`0 0 ${rect.width} ${rect.height}`);mobile.active=isMobileView();const prev={x:organism.x,y:organism.y};organism.x=rect.width/2;organism.y=rect.height/2+6;const m=Math.min(rect.width,rect.height);rings.events=mobile.active?clamp(m*0.24,92,150):206;rings.amino=mobile.active?clamp(m*0.38,145,230):320;rings.spectrum=mobile.active?clamp(m*0.16,70,108):116;rings.spectrumOuter=mobile.active?clamp(m*0.22,98,145):160;if(state?.amino?.length){const dx=organism.x-prev.x,dy=organism.y-prev.y;applyPan(dx,dy);state.amino.forEach((a,i)=>{if(!a.anchor&&!a.dragging){a.orbitRadius=rings.amino+(i%2?10:-8);}});}}
...
function resetState(){entitiesEl.innerHTML='';floatersEl.innerHTML='';linksSvg.innerHTML=`<defs><linearGradient id="mixLine" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stop-color="#7fd0ff"/><stop offset="52%" stop-color="#7fd0ff"/><stop offset="100%" stop-color="#ff9f36"/></linearGradient></defs>`;state.running=false;state.ended=false;state.startedAt=0;state.health=80;state.energy=60;state.collapse=10;state.events=[];state.sprouts=[];state.memories=[];state.memoryPoints=[];state.lastSpawn=0;state.dragging=null;state.memoryMode=false;stage.classList.remove('memoryMode');state.stats={spawned:0,revealed:0,resolved:0,transformed:0,sprouts:0,reinforcedLinks:0,typesResolved:{}};memoryDetails.innerHTML='';memoryIntro.textContent='Al terminar el reto, selecciona un punto naranja para leer qué ocurrió, cuándo ocurrió y cómo fue resuelto.';initAmino();initParticles();syncHud();clockEl.textContent='03:00';statusText.textContent='Pulsa iniciar. En móvil: arrastra aminoácidos; arrastra el fondo para desplazar el lienzo.';}

function onAminoDown(amino,e){if(state.memoryMode)return;e.preventDefault();e.stopPropagation();mobile.panning=false;if(amino.anchor){releaseAmino(amino);} amino.dragging=true;state.dragging=amino;amino.el.classList.add('selected');const pt=pointInStage(e);state.dragOffset.x=pt.x-amino.x;state.dragOffset.y=pt.y-amino.y;amino.el.setPointerCapture?.(e.pointerId);}
stage.addEventListener('pointerdown',e=>{if(state.memoryMode)return;const target=e.target;if(target!==stage && target!==bg && target!==linksSvg && target!==entitiesEl)return;e.preventDefault();const p=pointInStage(e);mobile.panning=true;mobile.last=p;stage.classList.add('is-panning');});
window.addEventListener('pointermove',e=>{const p=pointInStage(e);state.pointer.x=p.x;state.pointer.y=p.y;state.pointer.active=true;if(mobile.panning&&!state.dragging){const dx=p.x-mobile.last.x,dy=p.y-mobile.last.y;applyPan(dx,dy);mobile.last=p;return;}if(!state.dragging)return;const a=state.dragging;a.x=p.x-state.dragOffset.x;a.y=p.y-state.dragOffset.y;a.vx=0;a.vy=0;tryAutoAnchor(a);});
stage.addEventListener('pointerleave',()=>{state.pointer.active=false;state.pointer.x=-9999;state.pointer.y=-9999;});
window.addEventListener('pointerup',()=>{if(mobile.panning){mobile.panning=false;stage.classList.remove('is-panning');}if(!state.dragging)return;state.dragging.el.classList.remove('selected');state.dragging.dragging=false;state.dragging=null;});
...
function createMemoryPoint(memory){const el=document.createElement('button');el.type='button';el.className='memoryPoint';el.style.left=`${memory.x}px`;el.style.top=`${memory.y}px`;el.title=`${memory.event} · ${memory.time}`;el.addEventListener('pointerdown',(e)=>{e.stopPropagation();selectMemory(memory.id);});entitiesEl.appendChild(el);state.memoryPoints.push({id:memory.id,el});}
...
function updateDomForEvents(){state.events.forEach(ev=>{ev.el.style.transform=`translate(${ev.x}px,${ev.y}px)`;ev.el.classList.toggle('latent',ev.state==='latent');ev.el.classList.toggle('hiddenLabel',ev.state==='latent');});state.memoryPoints.forEach(p=>{const m=state.memories.find(item=>item.id===p.id);if(m){p.el.style.left=`${m.x}px`;p.el.style.top=`${m.y}px`;}});}
...
restartMemoryBtn.addEventListener('click',()=>{resetState();state.running=true;state.startedAt=nowMs();state.lastSpawn=nowMs()-state.spawnGap;statusText.textContent='Reto activo. Arrastra aminoácidos; desplaza el fondo si necesitas alcanzar una órbita.';});
